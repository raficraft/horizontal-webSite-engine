class ScrollHorizontalManager {
  ...
  constructor(params = {}) {
    ...
    this.slideWrapper = document.querySelector(
      `[data-${this.params.sliderName}]`
    );
    ...
    if (
      document.querySelector(`[data-${this.params.sliderName}prev]`) &&
      document.querySelector(`[data-${this.params.sliderName}next]`)
    ) {
      this.buttonPrev = document.querySelector(
        `[data-${this.params.sliderName}prev]`
      );
      this.buttonNext = document.querySelector(
        `[data-${this.params.sliderName}next]`
      );
  ...
  drawSliderPage() {
    ...
    for (const key in this.slideCollection) {
      if (Object.hasOwnProperty.call(this.slideCollection, key)) {
        ...
        if (
          parseInt(key) === parseInt(this.nbSlide) - 1 &&
          this.params.infiniteLoop === false &&
          this.params.slideWidth < 100
        ) {
    ...
    if (this.params.slideLink === true) {
      for (const key in this.linkCollection) {
        if (Object.hasOwnProperty.call(this.linkCollection, key)) {
          ...
          link.setAttribute(
            "href",
            `${this.params.sliderName}_${parseInt(key) + this.offset}`
          );
        }
      }
    }
  ...
  infiniteLoop() {
    ...
    this.slideCollection = [
      ...slideCollection
        .slice(this.nbSlide - this.offset)
  ...
  linkToggleClass(idx) {
    ...
    const thisLinkActive = document.querySelector(
      `[href=${this.params.sliderName}_${idx}]`
    );
  ...
  resizeSlider() {
    ...
    if (
      this.params.pushUp !== false &&
      typeof this.params.pushUp === "string"
    ) {
